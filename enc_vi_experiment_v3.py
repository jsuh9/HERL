...
grid_size = 2 # for (gird_size - by - grid_size) grid world
# goal_state_loc = (2, 2) # (2, 2) grid is the goal
# trap_state_loc = (2, 0) # (2, 0) is trap
goal_state_loc = (1, 1) # (2, 2) grid is the goal
trap_state_loc = (1, 0) # (2, 0) is trap
...
params_set_meta = []
ciph_modulus_set = [100]
Nslots = degree // 2 # number of slots for messaages

T = 30  # number of iterations
z_star, z_final, errors = recursive_linear_system(A, w, Nslots, T, tolerance=1e-10)

scaling_factors_set = [34]  # Bits for scaling_factor
# scaling_factors_set = [36]  # Bits for scaling_factor
# scaling_factors_set = [42]  # Bits for scaling_factor
# scaling_factors_set = [48]  # Bits for scaling_factor
...
for experiment_num in range(len(params_set_meta)):
    ...
    enc_error = np.mean(np.abs(ciph_sum_decoded_real - z_final)) / np.mean(z_star)  # Expected error
    ...
    for t in tqdm(range(1, (T + 1 + 5)), "Iterating over FHE..."):
        ...
        # check termination
        if enc_error < 1e-10:
            print(f"Converged at iteration {t} with (encrypted) error {enc_error} < {1e-10}.")
            break

    enc_errors_list.append(enc_errors)

error_data = {
    'errors': errors, # true errors (unencrypted)
    'enc_errors': enc_errors,
    'scaling_factors': scaling_factors_set,
    'poly_degree': degree,
    'T': T
}

np.save(f'saved_errors_data_{scaling_factors_set}.npy', error_data)